...
/*
 * Adaptive background priority sched_ext scheduler.
 *
 * Dynamically switches priority between flush and L0 compaction:
 *
 * FLUSH priority (class_id=1 with memtable pressure):
 *   - stall_flag set, OR
 *   - num_immutable_memtables >= imm_memtables_thr, OR
 *   - cur_size_all_memtables >= memtable_bytes_thr
 *
 * L0 priority (class_id=2, start_level=0, l0_files >= l0_files_thr):
 *   - only when L0 compaction is the bottleneck
 *
 * Dispatch order: FLUSH_DSQ > L0_DSQ > COMP_DSQ > SHARED_DSQ
 *
 * Budgets prevent starvation:
 *   flush_budget: after N flush dispatches, force L0/COMP/SHARED
 *   l0_budget:    after N L0 dispatches, force COMP/SHARED
 */
...
/*
 * Flush can preempt any non-flush task (prio_state != CPU_PRIO_FLUSH)
 * that has p->mm.
 */
...
s32 BPF_STRUCT_OPS(adaptive_bg_prio_select_cpu, struct task_struct *p,
...
void BPF_STRUCT_OPS(adaptive_bg_prio_enqueue, struct task_struct *p, u64 enq_flags)
{
...
void BPF_STRUCT_OPS(adaptive_bg_prio_dispatch, s32 cpu, struct task_struct *prev)
{
...
void BPF_STRUCT_OPS(adaptive_bg_prio_running, struct task_struct *p)
{
...
void BPF_STRUCT_OPS(adaptive_bg_prio_stopping, struct task_struct *p, bool runnable)
{
	u32 cpu = bpf_get_smp_processor_id();
	u32 zero = 0;
...
void BPF_STRUCT_OPS(adaptive_bg_prio_enable, struct task_struct *p)
{
	p->scx.dsq_vtime = vtime_now;
}

s32 BPF_STRUCT_OPS_SLEEPABLE(adaptive_bg_prio_init)
{
	s32 ret;
...
void BPF_STRUCT_OPS(adaptive_bg_prio_exit, struct scx_exit_info *ei)
{
	UEI_RECORD(uei, ei);
}

SCX_OPS_DEFINE(adaptive_bg_prio_ops,
	       .select_cpu	= (void *)adaptive_bg_prio_select_cpu,
	       .enqueue		= (void *)adaptive_bg_prio_enqueue,
	       .dispatch	= (void *)adaptive_bg_prio_dispatch,
	       .running		= (void *)adaptive_bg_prio_running,
	       .stopping	= (void *)adaptive_bg_prio_stopping,
	       .enable		= (void *)adaptive_bg_prio_enable,
	       .init		= (void *)adaptive_bg_prio_init,
	       .exit		= (void *)adaptive_bg_prio_exit,
	       .name		= "adaptive_bg_prio");
